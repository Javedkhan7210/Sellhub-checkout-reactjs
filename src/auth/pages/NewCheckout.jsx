...
export default function NewCheckout() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const response = await fetch(
          `https://apipayment.sellhub.net/api/checkout/product/${productId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      const token = sessionStorage.getItem("seller_token");
      const response = await fetch(
        `https://apipayment.sellhub.net/api/checkout/payment-check-promo`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleContinueToPayment = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://apipayment.sellhub.net/api/checkout/payment/${productId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        if (data?.success) {
          ...
          const totalAmount = productPrice + taxAmount - discountAmount;
          ...
          if (gateway?.name === "Tap Payment") {
            ...
            navigate(`/auth/payment/${data.transaction_id}`, {
              state: {
                url: data.url,
                Ids: productId,
                discount: appliedCoupon?.discount,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          ...
          setPaymentMessage("Payment approved successfully!");
          setLoaderStatus("success");

          // Calculate all values before navigation
          const productPrice = Number(product?.price) || 0;
          const taxAmount = Number(tax) || 0;
          const discountAmount = Number(transactionData?.discount) || 0;
          const totalAmount = productPrice + taxAmount - discountAmount;
          ...
          setPaymentMessage(data.message || "Payment was declined.");
          setLoaderStatus("processing");
          setShowSuccessModal(false);
          // Wait 3 seconds then navigate
          setTimeout(() => {
            ...
            navigate("/auth/payment-declined", {
              state: {
                ...
                reason: data.reason_of_decline || "Payment was not successful.",
                details: data,
              },
              replace: true,
            });
          }, 3000);
          // toast.error(data.message || "Payment was declined.");
              ...
              <div className="space-y-4">
                <div className="flex space-x-4">
                  ...
                  <Button
                    ...
                    className="h-11 px-8 bg-gradient-to-r from-[#f4b72d] to-[#f4b72d] hover:from-[#d89c18] hover:to-[#c58910] text-white font-semibold rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl"
                  >
                    {couponLoading ? (
                      <>
                ...
                {appliedCoupon && (
                  <div className="bg-gradient-to-r from-yellow-900/20 to-yellow-800/20 border border-yellow-600/30 rounded-xl p-4 backdrop-blur-sm">
...
`}
            ...
            <div className="sticky bottom-6 rounded-xl p-2 shadow-2xl mr-5 ">
              ...
              {/* <Button
                ...
                disabled={isSubmitting}
                className="w-full h-12 bg-gradient-to-r from-yellow-600 to-yellow-700 hover:from-yellow-700 hover:to-yellow-800 text-black text-lg font-bold rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105"
              >
                {isSubmitting ? (
                  <>
                    <Loader2 className="w-5 h-5 mr-3 animate-spin" />
              ...
              {/* <div className="bg-[rgb(24_24_27/_0.5)] backdrop-blur-sm rounded-xl shadow-lg p-4 relative z-10 w-full">
                ...
                <div className="space-y-4">
                  <div className="flex items-start space-x-3">
                    {product?.image && (
                      <img
                        src={`https://apipayment.sellhub.net/${product.image}`}
                        alt={product.title}
                        className="w-16 h-16 object-cover rounded-lg border border-gray-700 shadow-lg"
                      />
                    )}
                    <div className="flex-1">
                      <h4 className="font-semibold text-yellow-100 line-clamp-2">
                        {product?.title}
                      </h4>
